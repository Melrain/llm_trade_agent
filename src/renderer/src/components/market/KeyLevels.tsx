...
import { formatNum, levelLabel } from '@/lib/format'
import type { MarketLevel } from '../../../../preload/market-types'

export function KeyLevels({ levels }: { levels: MarketLevel[] }): JSX.Element {
  return (
        ...
        <ul className="mt-2 divide-y divide-border">
          {levels.map((level) => (
            <li key={level.id} className="grid grid-cols-4 items-center gap-2 py-1.5 text-[13px]">
              ...
              <span className="tabular-nums">{formatNum(level.high)}</span>
              <span className="tabular-nums">{formatNum(level.low)}</span>
